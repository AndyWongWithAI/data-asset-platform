...
**当前状态：三期+四期+五期+六期+七期+八期已实现并合入 `main`**（生产态三看板〔数据质量看板 / 数据血缘看板 / 数据标准看板〕+ 设计态 M1-M5〔数据资产目录 / 数据质量 / 数据标准〔可收缩目录：基础术语 / 值域 / 参考数据 / 信息项〕/ 数据安全 / 主数据〕+ 数据交换模块〔批次文件 / 数据服务〕，全部只读展示 + 占位写按钮；血缘看板 UML 类图（表级+字段级双视图，点字段看字段级血缘，交换方式标注在连线上）；六期数据标准细化为数据元四层体系〔基础术语 28 词根 / 值域 6 条 / 参考数据 5 条 / 信息项 10 条〕；七期数据标准加可收缩目录、基础术语去下钻、信息项加类型（技术/业务）/业务域/定义、字段明细「关联标准」点击转跳信息项、贯标改为「关联标准 + 中英文名精确对齐」判定）；八期将 7 个「列表+详情」模块（信息项/值域/参考数据/质量/主数据/批次文件/数据服务）的详情拆为独立详情 tab（总览/详情双 tab，单实例复用）。九期（写操作）P0 后端已实现：`server/` Express + JSON 文件落盘（从简，无数据库），7 元数据实体（基础术语/值域/参考数据/信息项/质量规则/主数据可 POST 新增，安全分级可 PUT 修改），通用 CRUD + 结构校验（必填/唯一/枚举/类型/引用完整性），前端尚未接入（P1）。本仓库 `main` 受 git-guardrails 保护（只接受 merge 提交），开发走任务分支再 merge。
