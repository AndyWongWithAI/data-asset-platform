...
**当前状态：三期+四期+五期+六期已实现并合入 `main`**（生产态三看板〔数据质量看板 / 数据血缘看板 / 数据标准看板〕+ 设计态 M1-M5〔数据资产目录 / 数据质量 / 数据标准〔基础术语 / 值域 / 参考数据 / 信息项〕/ 数据安全 / 主数据〕+ 数据交换模块〔批次文件 / 数据服务〕，全部只读展示 + 占位写按钮；血缘看板 UML 类图（表级+字段级双视图，点字段看字段级血缘，交换方式标注在连线上）；六期数据标准细化为数据元四层体系〔基础术语 28 词根 / 值域 6 条 / 参考数据 5 条 / 信息项 10 条〕）。本仓库 `main` 受 git-guardrails 保护（只接受 merge 提交），开发走任务分支再 merge。
...
- **开发优先级**：首期 MVP = M1 + M0；二期 M2/M3/M4/M5 独立页；三期 M0 拆三看板 + 血缘看板 + 批次文件 + 数据服务（原 M6）；四期血缘看板升级 UML 类图（表级+字段级双视图 + 交换方式标注移位）；五期血缘看板字段级血缘升级 N 级多跳遍历；六期数据标准细化为基础术语/值域/参考数据/信息项四模块。全部已实现并合入 main。
