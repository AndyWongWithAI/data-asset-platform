...
**当前状态：三期已实现并合入 `main`**（生产态三看板〔数据质量看板 / 数据血缘看板 / 数据标准看板〕+ 设计态 M1-M5〔数据资产目录 / 数据质量 / 数据标准 / 数据安全 / 主数据〕+ 数据交换模块〔批次文件 / 数据服务〕，全部只读展示 + 占位写按钮；血缘看板纯 SVG 手绘）。本仓库 `main` 受 git-guardrails 保护（只接受 merge 提交），开发走任务分支再 merge。
...
| M6 数据服务 | 封装 M1 治理达标资产为产品/API（列表 + 审批链 + 计量） | P1 |
| 批次文件 | 离线批次交换任务（列表 + 审批链 + 源表转跳） | P1 |
