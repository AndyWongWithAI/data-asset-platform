...
**门户 SPA（新，`src/portal/`）：**
- `src/portal/index.html` — 门户 HTML 入口（门户独立 root，base `/data-asset-portal/`）。
- `vite.portal.config.js` — 门户 Vite config（base + root src/portal + outDir ../../dist-portal + dev 端口 5174）。
...
**Files:**
- Create: `src/portal/index.html`、`vite.portal.config.js`、`src/portal/main.jsx`、`src/portal/PortalApp.jsx`、`src/portal/PortalHeader.jsx`、`src/portal/PortalCatalog.jsx`、`src/portal/portal.css`
- Modify: `package.json`
...
创建 `vite.portal.config.js`（**门户用独立 root `src/portal`，这样入口是 `src/portal/index.html`、dev 在 `/data-asset-portal/` 根路径直接 serve、build 产出 `dist-portal/index.html`**，与 nginx `try_files ... index.html` 兜底对齐）：
...
创建 `src/portal/index.html`（root 为 src/portal，故 script src 用 `/main.jsx`）：
...
git add vite.portal.config.js package.json src/portal/
